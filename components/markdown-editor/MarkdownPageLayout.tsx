...
import { defaultMarkdownContent } from "@/data/markdownDefaults"
      ...
      {isMobile ? (
        <Tabs defaultValue="editor" className="flex-1 overflow-hidden">
          <TabsList className="w-full justify-start rounded-none border-b border-gray-200 bg-transparent">
            <TabsTrigger
              value="editor"
              className="rounded-none data-[state=active]:border-b-2 data-[state=active]:border-gray-900"
            >
              편집기
            </TabsTrigger>
            <TabsTrigger
        ...
        <div className="flex flex-1 overflow-hidden">
          <EditorPanel markdown={markdown} onChange={setMarkdown} className="w-1/2 border-r border-gray-200" />
          <PreviewPanel markdown={debouncedMarkdown} className="w-1/2" />
        </div>
      )}

      <StatusBar wordCount={wordCount} charCount={charCount} isSaved={isSaved} />
    </div>
  )
}
